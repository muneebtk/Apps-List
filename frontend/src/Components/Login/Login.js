...
function Login() {
  let { signupSuccessRes,LoginUser,loginRes,signupRes, loginErrorRes } = useContext(AppContext);
    ...
    <div>
      <Grid>
        <Paper className="paper" elevation={0} variant="outlined">
          ...
          <Grid>
            {loginRes && (
              <Alert severity="error">{loginRes.detail}</Alert>
            ) }

            {/* {signupSuccessRes ? (
              <Alert severity="success">{signupSuccessRes}</Alert>
            ) : null} */}
